...
export class ActiveHashRecord implements ActiveHashRecordBase {
    ...
    protected hasMany<Record extends ActiveHashRecord>(
        target: ActiveHash<Record>,
        foreignKey?: keyof Record,
    ...
    protected hasOne<Record extends ActiveHashRecord>(target: ActiveHash<Record>, foreignKey?: keyof Record) {
        const useForeignKey = foreignKey || `${snakeCase(this._parentTable.name)}_id`;
        return target.findBy({ [useForeignKey]: this.id } as any);
    }

    protected belongsTo<Record extends ActiveHashRecord>(target: ActiveHash<Record>, foreignKey?: keyof this) {
